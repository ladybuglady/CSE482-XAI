...
def combine_sets(afib_path, control_path, size=20000):
  ...
  y_labels_afib = np.ones(20000) # an array of all 1's

  # Now, let's get the healthy patients:
  control_ecgs = random.choices(os.listdir(control_path), k=20000)
  y_labels_control = np.zeros(20000)
  ...
  random_indices = np.random.randint(low=0,high=20000*2-1,size = (20000*2,))
...
def fetch_data(datatype, size=20000):
