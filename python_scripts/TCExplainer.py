...
class TimeConsistentExplainer(Explainer):
    #TIME Consistent Explainer, just changest masking routine for kernelshap
        ...
        # if more than one feature varies then we have to do real work
            ...
            for subset_size in range(1, num_subset_sizes + 1):
                ...
                if num_samples_left * remaining_weight_vector[subset_size - 1] / nsubsets >= 1.0 - 1e-8:
                    ...
                    #Edited the indices to be lists instead of random choices


                    inds = list(range(self.M/2-subset_size/2, self.M/2)) + list(range(self.M - subset_size/2, self.M))
                    mask[:] = 0.0
